    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border p-5">
                <h2 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Versi&oacute;n Actual</h2>
                <p class="text-4xl font-bold text-indigo-600">v{{ $currentVersion }}</p>
                @if($lastChecked)
                    <p class="text-xs text-gray-500 mt-2">&Uacute;ltima verificaci&oacute;n: {{ $lastChecked }}</p>
                @endif
                <button
            ...
            @if($updateAvailable && $latestRelease)
                <div class="bg-green-50 border border-green-200 rounded-lg p-5">
                    <div class="flex items-center mb-3">
                        ...
                        <h3 class="text-sm font-semibold text-green-800">Nueva Versi&oacute;n</h3>
                    </div>
                    <p class="text-2xl font-bold text-green-700 mb-1">v{{ $latestRelease['version'] }}</p>
                    @if($latestRelease['published_at'])
                        <p class="text-xs text-green-600 mb-4">{{ \Carbon\Carbon::parse($latestRelease['published_at'])->format('d/m/Y') }}</p>
                    @endif
                    <button
            ...
            @elseif($latestVersion && !$updateAvailable && !$error)
                <div class="bg-gray-50 border border-gray-200 rounded-lg p-5">
                    <div class="flex items-center">
                        ...
                        <h3 class="text-sm font-semibold text-gray-700">Sistema Actualizado</h3>
                    </div>
                    <p class="text-xs text-gray-500 mt-2">Ya tienes la &uacute;ltima versi&oacute;n disponible.</p>
                </div>
            @endif
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @if($updateAvailable && $latestRelease && $latestRelease['changelog'])
                <div class="bg-white rounded-lg shadow-sm border p-5">
                    <h3 class="text-sm font-semibold text-gray-800 mb-3">Notas de la Versi&oacute;n v{{ $latestRelease['version'] }}</h3>
                    <div class="bg-gray-50 rounded-lg p-4 text-sm text-gray-700 prose prose-sm max-w-none max-h-48 overflow-y-auto">
            ...
            <div class="bg-white rounded-lg shadow-sm border p-5">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h2 class="text-sm font-semibold text-gray-800">Backups Disponibles</h2>
                        <p class="text-xs text-gray-500">Copias de seguridad antes de actualizar</p>
                    </div>
                    <button
                        wire:click="createBackup"
                        wire:loading.attr="disabled"
                        wire:target="createBackup"
                        class="inline-flex items-center px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition text-xs"
                    >
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tama&ntilde;o</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Archivo</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($backups as $backup)
                                    <tr>
                                        <td class="px-3 py-2 whitespace-nowrap text-xs text-gray-900">{{ $backup['date'] }}</td>
                                        <td class="px-3 py-2 whitespace-nowrap text-xs text-gray-500">{{ $backup['size'] }}</td>
                                        <td class="px-3 py-2 whitespace-nowrap text-xs text-gray-500 font-mono truncate max-w-xs">{{ $backup['name'] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
