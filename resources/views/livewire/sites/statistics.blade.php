<div class="w-full space-y-6">
    ...
    @if(!$stats['site'])
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-1">Sitio no encontrado</h3>
            <p class="text-gray-500 mb-4">El sitio solicitado no existe o ha sido eliminado.</p>
            <a href="{{ route('sites.index') }}" class="text-blue-600 hover:text-blue-800 font-medium">
                Volver a sitios
            </a>
        </div>
    @else
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4">
                <a href="{{ route('sites.index') }}" class="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                    </svg>
                </a>
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ $stats['site']->name }}</h1>
                    <p class="text-gray-500 text-sm">{{ $stats['site']->domain }}</p>
                </div>
            </div>
                ...
                <div class="flex bg-gray-100 rounded-lg p-1">
                    @foreach(['7d' => '7 dias', '30d' => '30 dias', '90d' => '90 dias', '12m' => '12 meses', 'all' => 'Todo'] as $key => $label)
                        <button wire:click="setDateRange('{{ $key }}')"
                                class="px-3 py-1.5 text-xs font-medium rounded-md transition {{ $periodPreset === $key ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                            {{ $label }}
                        </button>
                    @endforeach
                </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['totals']['leads']) }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['totals']['deals']) }}</p>
                        <p class="text-xs text-gray-500">Negocios</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-green-600">{{ number_format($stats['totals']['won']) }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-red-600">{{ number_format($stats['totals']['lost']) }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-gray-900">${{ number_format($stats['totals']['value'], 0) }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 class="text-sm font-semibold text-gray-900 mb-4">Tasa de Conversion</h3>
                <div class="flex items-center justify-center">
                    <div class="relative w-40 h-40">
                        ...
                        <div class="absolute inset-0 flex items-center justify-center">
                            <span class="text-3xl font-bold text-gray-900">{{ $stats['conversion_rate'] }}%</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:col-span-2">
                <h3 class="text-sm font-semibold text-gray-900 mb-4">Distribucion por Fuente</h3>
                        ...
                        @foreach($stats['leads_by_source'] as $source => $count)
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1">
                                    <span class="text-sm font-medium text-gray-700">{{ $this->getSourceTypeLabel($source) }}</span>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 class="text-sm font-semibold text-gray-900 mb-4">Leads en el Tiempo</h3>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <!-- Deals by Phase -->
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 class="text-sm font-semibold text-gray-900 mb-4">Negocios por Fase</h3>
                    ...
                    <div class="space-y-3">
                        @foreach($stats['deals_by_phase'] as $phase)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                ...
                                <span class="text-sm font-semibold text-gray-900">{{ $phase['count'] }}</span>
                            </div>
                        @endforeach
                    </div>
                @else
            ...
            <!-- Recent Leads -->
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 class="text-sm font-semibold text-gray-900 mb-4">Leads Recientes</h3>
                @if($stats['recent_leads']->count() > 0)
                    <div class="space-y-3 max-h-80 overflow-y-auto">
                        @foreach($stats['recent_leads'] as $lead)
                            <a href="{{ route('leads.show', $lead->id) }}"
                               target="_blank"
                               class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-blue-50 hover:border-blue-200 border border-transparent transition group">
                                <div class="flex items-center gap-3 min-w-0">
                                    <div class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0 group-hover:bg-blue-200 transition">
                                        <span class="text-xs font-semibold text-blue-600">
                                            {{ strtoupper(substr($lead->name ?? 'L', 0, 1)) }}
                                        </span>
                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-sm font-medium text-gray-900 truncate group-hover:text-blue-700 transition">{{ $lead->name ?? 'Sin nombre' }}</p>
                                        <p class="text-xs text-gray-500">{{ $lead->phone ?? $lead->email ?? '-' }}</p>
                                    </div>
                                </div>
                                    ...
                                    <div>
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium
                                            {{ $lead->source_type->value === 'whatsapp_button' ? 'bg-green-100 text-green-700' :
                                               ...
                                               ($lead->source_type->value === 'contact_form' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-700')) }}">
