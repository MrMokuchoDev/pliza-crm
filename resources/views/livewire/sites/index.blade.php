                        ...
                        <div class="p-5">
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-lg flex items-center justify-center {{ $site->is_active ? 'bg-green-100' : 'bg-gray-100' }}">
                                        <svg class="w-5 h-5 {{ $site->is_active ? 'text-green-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $site->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                            ...
                            <div class="flex items-center gap-4 mb-4 text-sm">
                                <a href="{{ route('sites.statistics', $site->id) }}"
                                   class="flex items-center gap-1.5 text-gray-600 hover:text-blue-600 transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
                    <div class="bg-white px-6 pt-6 pb-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900">Eliminar Sitio</h3>
                                <p class="text-sm text-gray-500 mt-1">Esta acci&oacute;n no se puede deshacer. Los widgets dejar&aacute;n de funcionar.</p>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3">
                ...
                <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    <div class="bg-white px-6 pt-6 pb-4">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900">C&oacute;digo de Integraci&oacute;n</h3>
                            <span class="text-sm text-gray-500">{{ $embedSite->name }}</span>
                        </div>

                        <!-- Widget Type Selector -->
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tipo de Widget</label>
                            <div class="grid grid-cols-3 gap-3">
                                <button type="button"
                                        wire:click="$set('selectedWidgetType', 'whatsapp')"
                                        class="p-3 rounded-lg border-2 transition {{ $selectedWidgetType === 'whatsapp' ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300' }}">
                                    <div class="w-8 h-8 mx-auto mb-2 {{ $selectedWidgetType === 'whatsapp' ? 'text-green-600' : 'text-gray-400' }}">
                                        <svg fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                                        </svg>
                                    </div>
                                    <span class="text-xs font-medium {{ $selectedWidgetType === 'whatsapp' ? 'text-green-700' : 'text-gray-600' }}">WhatsApp</span>
                                </button>

                                <button type="button"
                                        wire:click="$set('selectedWidgetType', 'phone')"
                                        class="p-3 rounded-lg border-2 transition {{ $selectedWidgetType === 'phone' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300' }}">
                                    <div class="w-8 h-8 mx-auto mb-2 {{ $selectedWidgetType === 'phone' ? 'text-blue-600' : 'text-gray-400' }}">
                                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                                        </svg>
                                    </div>
                                    <span class="text-xs font-medium {{ $selectedWidgetType === 'phone' ? 'text-blue-700' : 'text-gray-600' }}">Llamada</span>
                                </button>

                                <button type="button"
                                        wire:click="$set('selectedWidgetType', 'contact_form')"
                                        class="p-3 rounded-lg border-2 transition {{ $selectedWidgetType === 'contact_form' ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-gray-300' }}">
                                    <div class="w-8 h-8 mx-auto mb-2 {{ $selectedWidgetType === 'contact_form' ? 'text-purple-600' : 'text-gray-400' }}">
                                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                        </svg>
                                    </div>
                                    <span class="text-xs font-medium {{ $selectedWidgetType === 'contact_form' ? 'text-purple-700' : 'text-gray-600' }}">Formulario</span>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 p-4 bg-blue-50 rounded-lg">
                            ...
                            <ol class="text-sm text-blue-800 space-y-1 list-decimal list-inside">
                                ...
                                <li>P&eacute;galo antes del cierre de <code class="bg-blue-100 px-1 rounded">&lt;/body&gt;</code> en tu sitio web</li>
