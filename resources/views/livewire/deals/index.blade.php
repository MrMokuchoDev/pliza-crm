            ...
            <div class="flex flex-col lg:flex-row lg:items-center gap-3">
                ...
                <div class="relative lg:w-64">
                    ...
                    <input type="text"
                           ...
                           class="w-full pl-9 pr-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500">
                </div>

                <!-- Filter by Phase -->
                <select wire:model.live="filterPhase"
                        class="lg:w-48 px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                ...
                <div class="flex items-center gap-3 lg:gap-4 lg:ml-auto overflow-x-auto pb-1 lg:pb-0">
                    <div class="flex items-center gap-1.5 text-sm whitespace-nowrap">
                        ...
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $totalDeals }}</span>
                    </div>
                    <div class="flex items-center gap-1.5 text-sm whitespace-nowrap">
                        <span class="w-2 h-2 bg-orange-500 rounded-full flex-shrink-0"></span>
                        <span class="text-gray-500 dark:text-gray-400 hidden sm:inline">Abiertos:</span>
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $openDeals }}</span>
                    </div>
                    @if($totalValue > 0)
                        <div class="flex items-center gap-1.5 text-sm whitespace-nowrap">
                            <span class="w-2 h-2 bg-green-500 rounded-full flex-shrink-0"></span>
                            <span class="text-gray-500 dark:text-gray-400 hidden sm:inline">Valor:</span>
                            <span class="font-semibold text-green-600 dark:text-green-400">${{ number_format($totalValue, 0, ',', '.') }}</span>
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Valor</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Fase</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <a href="{{ route('deals.show', $deal->id) }}" class="font-medium text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition">{{ $deal->name }}</a>
                                            @if($deal->description)
                                                <p class="text-xs text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ Str::limit($deal->description, 40) }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($deal->lead)
                                        <div class="flex flex-col gap-1">
                                            <a href="{{ route('leads.show', $deal->lead->id) }}" class="font-medium text-gray-900 dark:text-white text-sm hover:text-blue-600 dark:hover:text-blue-400">
                                                {{ $deal->lead->name ?? 'Sin nombre' }}
                                            </a>
                                            @if($deal->lead->phone)
                                                <div class="flex items-center gap-2">
                                                    <span class="text-xs text-gray-500 dark:text-gray-400">{{ $deal->lead->phone }}</span>
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-600 dark:text-gray-300">{{ $deal->created_at->format('d/m/Y') }}</div>
                                    <div class="text-xs text-gray-400 dark:text-gray-500">{{ $deal->created_at->format('H:i') }}</div>
                                </td>
                                <td class="px-6 py-4">
                ...
                @foreach($deals as $deal)
                    <div class="p-4 overflow-hidden">
                        ...
                        <div class="flex items-start justify-between mb-3">
                            <a href="{{ route('deals.show', $deal->id) }}" class="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div class="min-w-0 flex-1">
                                    <h3 class="font-semibold text-gray-900 dark:text-white truncate">{{ $deal->name }}</h3>
                                    <div class="flex items-center gap-2 mt-0.5 flex-wrap">
                                        @if($deal->value)
                                            <span class="font-semibold text-green-600 dark:text-green-400 text-sm">{{ $deal->formatted_value }}</span>
                                        @endif
                        ...
                        <div class="flex items-center justify-between">
                            @if($deal->lead)
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('leads.show', $deal->lead->id) }}" class="text-sm text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400">
                                        {{ $deal->lead->name ?? 'Sin nombre' }}
                                    </a>
                                </div>
                                @if($deal->lead->phone)
                                    <div class="flex items-center gap-2">
                                        <a href="tel:{{ $deal->lead->phone }}"
                                           class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg text-sm font-medium">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $deal->lead->phone) }}"
                                           target="_blank"
                                           class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg text-sm font-medium">
        ...
        @else
            <div class="p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-1">No hay negocios todav&iacute;a</h3>
                <p class="text-gray-500 dark:text-gray-400 mb-4">Crea tu primer negocio para comenzar a gestionar tu pipeline</p>
                @if($canCreate)
                <button wire:click="openCreateModal"
    ...
    @if($showDeleteModal)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-80 transition-opacity" wire:click="closeDeleteModal"></div>
                    ...
                    <div class="bg-white dark:bg-gray-800 px-6 pt-6 pb-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Esta acci&oacute;n no se puede deshacer.</p>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-700/50 px-6 py-4 flex justify-end gap-3">
                        <button wire:click="closeDeleteModal" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-500">Cancelar</button>
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
                    <form wire:submit="confirmWonWithValue">
                        <div class="bg-white dark:bg-gray-800 px-6 pt-6 pb-4">
                            ...
                            <div class="mt-4">
                                <label for="dealValueIndex" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
