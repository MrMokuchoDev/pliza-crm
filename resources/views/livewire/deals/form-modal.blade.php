                    ...
                    <form wire:submit="save">
                        <div class="bg-white px-6 pt-6 pb-4">
                            ...
                            @if($showLeadSearch && !$dealId)
                                <div class="mb-6">
                                    <h4 class="text-sm font-medium text-gray-700 mb-3">Seleccionar Contacto</h4>

                                    {{-- Search input --}}
                                    <div class="relative">
                                        <svg wire:loading.remove wire:target="leadSearch" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if(strlen($leadSearch) >= 2)
                                        <div class="mt-3 border border-gray-200 rounded-lg divide-y divide-gray-100 max-h-64 overflow-y-auto">
                                            @forelse($searchResults as $result)
                                                <button type="button"
                                                        wire:click="selectLead('{{ $result->id }}')"
                                                        class="w-full px-4 py-3 text-left hover:bg-gray-50 transition flex items-center justify-between gap-3 {{ $result->active_deals_count > 0 ? 'opacity-60' : '' }}">
                                                    <div class="flex items-center gap-3">
                                                        ...
                                                        <div class="min-w-0">
                                                            <p class="font-medium text-gray-900 truncate">{{ $result->name ?? 'Sin nombre' }}</p>
                                                    ...
                                                    @if($result->active_deals_count > 0)
                                                        <span class="text-xs px-2 py-1 bg-orange-100 text-orange-700 rounded-full flex-shrink-0">
                                                            Negocio activo
                                                        </span>
                                                    @else
                                            ...
                                            @empty
                                                <div class="px-4 py-6 text-center">
                                                    <p class="text-sm text-gray-500 mb-3">No se encontraron contactos</p>
                                                    <button type="button"
                                    ...
                                    @else
                                        <div class="mt-3 p-6 text-center border border-dashed border-gray-300 rounded-lg">
                                            <svg class="w-10 h-10 text-gray-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                                            </svg>
                                            <p class="text-sm text-gray-500 mb-4">Escribe al menos 2 caracteres para buscar contactos</p>
                                            <button type="button"
                                                    wire:click="startNewLead"
                            ...
                            @else
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="space-y-4">
                                        <h4 class="text-sm font-medium text-gray-500 uppercase tracking-wide border-b pb-2">Datos del Negocio</h4>
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        @if(($leadPhone || $leadEmail) && !$createNewLead)
                                            <div class="pt-2">
                                                <label class="block text-sm font-medium text-gray-500 mb-2">Acciones Rapidas</label>
                                                <div class="flex gap-2">
                                                    @if($leadPhone)
                                                        ...
                                                        <a href="https://wa.me/{{ $cleanPhone }}"
                                                           target="_blank"
                                                           class="inline-flex items-center px-3 py-1.5 bg-green-100 text-green-700 rounded-lg text-sm hover:bg-green-200 transition">
                                                        ...
                                                        <a href="tel:{{ $leadPhone }}"
                                                           class="inline-flex items-center px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg text-sm hover:bg-blue-200 transition">
                                                    ...
                                                    @if($leadEmail)
                                                        <a href="mailto:{{ $leadEmail }}"
                                                           class="inline-flex items-center px-3 py-1.5 bg-purple-100 text-purple-700 rounded-lg text-sm hover:bg-purple-200 transition">
                                                            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
