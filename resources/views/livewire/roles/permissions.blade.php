<div class="w-full" x-data="{
    ...
    <!-- Header -->
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 mb-4">
        <div class="p-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-xl font-bold text-gray-900 dark:text-white">Permisos por Rol</h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Configura los permisos de cada rol del sistema</p>
                </div>

                <!-- Role Selector + Actions -->
                <div class="flex items-center gap-3">
                    <label for="role-select" class="text-sm font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">Rol:</label>
                    <select id="role-select"
                            wire:model.live="selectedRoleId"
                            class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm min-w-[180px] bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
    ...
    @if($this->selectedRole)
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                <h2 class="font-semibold text-gray-900 dark:text-white">{{ ucfirst($this->selectedRole->name) }}</h2>
                                <!-- Botones Editar/Eliminar Rol -->
                                <button wire:click="openEditRoleModal('{{ $this->selectedRole->id }}')"
                                        class="p-1 text-gray-400 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded transition"
                                        title="Editar rol">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if($this->selectedRole->name !== 'admin')
                                    <button wire:click="confirmDeleteRole('{{ $this->selectedRole->id }}')"
                                            class="p-1 text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition"
                                            title="Eliminar rol">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $this->selectedRole->description ?? 'Sin descripcion' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <button @click="expandAll()"
                                class="px-3 py-1.5 text-xs font-medium text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition">
                            Expandir Todo
                        </button>
                        ...
                        <button wire:click="selectAll"
                                class="px-3 py-1.5 text-xs font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg transition">
                            Todos
                        </button>
                        <button wire:click="deselectAll"
                                class="px-3 py-1.5 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition">
                            Ninguno
                        </button>
                        <button wire:click="resetToDefault"
                                class="px-3 py-1.5 text-xs font-medium text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 hover:bg-amber-100 dark:hover:bg-amber-900/50 rounded-lg transition">
                            Defecto
                        </button>
                    </div>
                </div>
                ...
                <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                    <div class="flex items-center gap-4 text-sm">
                        <span class="text-gray-500 dark:text-gray-400">
                            <span class="font-semibold text-gray-900 dark:text-white">{{ count($rolePermissions) }}</span>
                            de
                            <span class="font-semibold text-gray-900 dark:text-white">{{ $this->totalPermissionsCount }}</span>
                            permisos activos
                        </span>
                        @if($hasChanges)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-full text-xs font-medium">
                    ...
                    <button @click="expandedGroups['{{ $group }}'] = !expandedGroups['{{ $group }}']"
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-lg flex items-center justify-center
                                {{ $isFullySelected ? 'bg-blue-100 dark:bg-blue-900/30' : ($isPartiallySelected ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-gray-100 dark:bg-gray-700') }}">
                                <svg class="w-5 h-5 {{ $isFullySelected ? 'text-blue-600 dark:text-blue-400' : ($isPartiallySelected ? 'text-amber-600 dark:text-amber-400' : 'text-gray-500 dark:text-gray-400') }}"
                                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    {!! $icon !!}
                                </svg>
                            </div>
                            <div class="text-left">
                                <h3 class="font-semibold text-gray-900 dark:text-white text-sm">{{ $group }}</h3>
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    <span class="{{ $isFullySelected ? 'text-blue-600 dark:text-blue-400 font-medium' : ($isPartiallySelected ? 'text-amber-600 dark:text-amber-400 font-medium' : '') }}">
                        ...
                        <div class="flex items-center gap-2">
                            <!-- Status badge -->
                            @if($isFullySelected)
                                <span class="px-2 py-0.5 text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full">Todos</span>
                            @elseif($isPartiallySelected)
                                <span class="px-2 py-0.5 text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-full">Parcial</span>
                            @else
                                <span class="px-2 py-0.5 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded-full">Ninguno</span>
                            @endif
                        ...
                        <div class="px-4 pb-3 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <div class="py-2 mb-1 flex justify-end">
                                <button wire:click="toggleGroup('{{ $group }}')"
                                        class="text-xs font-medium px-2 py-1 rounded transition
                                            {{ $isFullySelected ? 'text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20' : 'text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20' }}">
                            ...
                            <div class="space-y-1">
                                @foreach($permissions as $permission)
                                    <label class="flex items-center gap-3 p-2 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition group">
                                        <input type="checkbox"
                                               wire:click="togglePermission('{{ $permission->id }}')"
                                               {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }}
                                               class="w-4 h-4 text-blue-600 border-gray-300 dark:border-gray-600 rounded focus:ring-blue-500 cursor-pointer bg-white dark:bg-gray-700">
                                        <div class="flex-1 min-w-0">
                                            <span class="block text-sm text-gray-900 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition">
        ...
        @if($hasChanges)
            ...
            <div class="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur border-t border-gray-200 dark:border-gray-700 shadow-lg z-40">
                <!-- Changes Preview -->
                <div class="max-w-6xl mx-auto px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                        ...
                        <div class="flex-1 flex flex-wrap gap-3 text-xs">
                            @if($preview['addedCount'] > 0)
                                <div class="flex items-start gap-1">
                                    <span class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded font-medium whitespace-nowrap">
                            ...
                            @if($preview['removedCount'] > 0)
                                <div class="flex items-start gap-1">
                                    <span class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded font-medium whitespace-nowrap">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @else
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-1">Selecciona un rol</h3>
            <p class="text-gray-500 dark:text-gray-400">Elige un rol del selector para gestionar sus permisos</p>
        </div>
    @endif
    ...
    @if($showRoleModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-80 transition-opacity" wire:click="closeRoleModal"></div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text"
                                           id="roleName"
                                           wire:model="roleName"
                                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
                                           placeholder="ej: supervisor, soporte, etc."
                                           {{ $editingRoleId && $this->selectedRole?->name === 'admin' ? 'disabled' : '' }}>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Solo minusculas y guion bajo, sin espacios</p>
                                    @error('roleName') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>

                                <!-- Descripción -->
                                <div>
                                    <label for="roleDescription" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                        Descripcion
                                    </label>
                                    <input type="text"
                                           id="roleDescription"
                                ...
                                <!-- Nivel -->
                                <div>
                                    <label for="roleLevel" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                    ...
                                    <input type="number"
                                           ...
                                           {{ $editingRoleId && $this->selectedRole?->name === 'admin' ? 'disabled' : '' }}>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">1-99 (mayor = mas permisos). Admin es 100.</p>
    ...
    @if($showDeleteRoleModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-80 transition-opacity" wire:click="closeDeleteRoleModal"></div>
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                    Estas a punto de eliminar el rol <strong class="text-gray-900 dark:text-white">{{ $this->deletingRole?->name ?? 'desconocido' }}</strong>.
                                </p>
                            </div>
                        </div>
