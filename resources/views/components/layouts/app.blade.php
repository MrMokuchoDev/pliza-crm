        ...
        <aside class="fixed inset-y-0 left-0 z-40 w-64 sidebar-gradient transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:z-auto shadow-xl"
            ...
            <nav class="px-3 flex-1">
                ...
                <div class="space-y-1">
                    ...
                    <!-- Kanban -->
                    <a href="{{ route('kanban') }}"
                       class="nav-item flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-150 {{ request()->routeIs('kanban') ? 'nav-item-active text-blue-400' : 'text-gray-300' }}">
                        <div class="w-8 h-8 rounded-lg {{ request()->routeIs('kanban') ? 'bg-blue-500/20' : 'bg-white/5' }} flex items-center justify-center mr-3">
                            <svg class="w-4 h-4 {{ request()->routeIs('kanban') ? 'text-blue-400' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
